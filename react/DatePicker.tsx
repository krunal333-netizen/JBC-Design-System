...
export interface JBCDatePickerProps {
  label?: string;
  value?: string;
...
/**
 * JBC Date Picker
 * Desktop: Styled field that triggers native calendar.
 * Mobile: Native OS date picker fallback.
 */
export const JBCDatePicker: React.FC<JBCDatePickerProps> = ({
  label,
  value,
  onChange,
  disabled = false,
  ...
  const handleContainerClick = () => {
    if (!disabled && inputRef.current) {
      // Modern browsers support showPicker() to programmatically trigger the UI
  ...
  return (
    <div className={`flex flex-col gap-2 w-full ${className}`}>
      ...
      <div className="relative">
        ...
        {/* JBC Styled Visual Container (matches JBCDropdown styles) */}
        <div
        ...
        >
          <div className="flex items-center gap-3">
            <JBCIcon name="ri-calendar-line" size={20} className="opacity-60" />
